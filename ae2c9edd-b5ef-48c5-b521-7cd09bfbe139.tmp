# (ここに先ほどのStreamlitのコードをすべて貼り付けて実行)
import itertools
...
st.title("🎓 学園アイドルマスター メモリ抽選シミュレーター (拡張版)")
st.markdown(
    "固有スキル枠やカスタマイズ要素を含めたカードプールから、コンテストメモリーに"
    "抽選される可能性のある組み合わせを計算します。"
)
...
if "user_cards" not in st.session_state:
  st.session_state.user_cards = [
      ...
      {"name": "決意 (虹)", "type": "虹", "cost": 150, "category": "通常"},
      {
          "name": "サポートカード枠",
          "type": "サポカ+",
          "cost": 126,
      ...
      {"name": "低銀スキル", "type": "低銀", "cost": 30, "category": "通常"},
  ]

st.header("🎴 シナリオ中で取得したスキルカード一覧")
st.markdown(
    "固有スキルやサポートカード枠、カスタム済みのカードなどを追加・管理できます。"
)
...
with st.form("add_card_form"):
  ...
  with col1:
    c_name = st.text_input("カード名", value="新規カード")
  with col2:
    c_category = st.selectbox(
        "枠カテゴリ", options=["通常", "固有", "サポカ"]
    )
  with col3:
...
if st.session_state.user_cards:
  ...
  for i, card in enumerate(st.session_state.user_cards):
    col_a, col_b, col_c, col_d = st.columns([3, 1.5, 2, 1])
    with col_a:
      st.text(f"{i+1}. {card['name']}")
    with col_b:
      st.text(f"[{card['category']}]")
    with col_c:
      st.text(f"{card['type']} (コスト:{card['cost']})")
    with col_d:
  ...
  if not st.session_state.user_cards:
    ...
    if err:
      ...
      for idx, res in enumerate(results):
        combo_names = ", ".join([
            f"{c['name']} [{c['category']}] ({c['type']})" for c in res["combo"]
        ])
        st.write(
